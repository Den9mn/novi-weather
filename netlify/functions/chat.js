// Netlify Function: chat.js  (OpenAI Assistants v2 via REST API)
// Needs env vars: OPENAI_API_KEY, ASSISTANT_ID, WEATHER_KEY
...
exports.handler = async (event) => {
  try {
    ...
    const userMessage =
      body.message || "What's the weather like in Madrid?";
    ...
    await fetch(`${OPENAI_API}/threads/${thread.id}/messages`, {
      method: "POST",
      headers: HEADERS(OPENAI_KEY),
    ...
    // 4️⃣  Poll until run completes or requests tool output
    while (true) {
      ...
      if (run.status === "requires_action") {
        ...
        for (const tc of toolCalls) {
          if (
            ...
            // Parse location argument (default Madrid)
            let location = "Madrid";
            try {
            ...
            if (!sto.ok)
              throw new Error(`Submit tool outputs failed: ${sto.status}`);
          }
        }
      }
    ...
    // 5️⃣  Fetch the final assistant message
    ...
    if (!listRes.ok)
      throw new Error(`List messages failed: ${listRes.status}`);
